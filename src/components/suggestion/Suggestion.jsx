...
function Suggestion({ keyword, className }) {
    ...
    return (
        <div className={`bg-[#2d2b44] ${className} flex ${loading ? "justify-center py-7" : "justify-start"} ${!suggestion ? "p-3" : "justify-start"} items-center`}
            ...
            {loading ? (
                ...
                <div className='w-full flex flex-col pt-2 overflow-y-auto'>
                    {suggestion.map((item, index) => (
                        ...
                        >
                            <img src={item.imgData} className='w-[50px] h-[75px] flex-shrink-0 object-cover' alt="" onError={(e) => { e.target.src = "https://iili.io/2J1Rwnp.jpg" }} />
                            <div className='flex flex-col gap-y-[2px]'>
                                {item?.title && <h1 className='line-clamp-1 leading-5 font-bold text-[15px] group-hover:text-[#ffbade]'>{item.title || 'N/A'}</h1>}
                                {item?.japanese_title && <h1 className='line-clamp-1 leading-5 text-[13px] font-light text-[#aaaaaa]'>{item.japanese_title || 'N/A'}</h1>}
                                {(item?.releaseDate || item?.showType || item?.duration) && (
                                    <div className='flex gap-x-[5px] items-center w-full justify-start mt-[4px]'>
                                        <p className='leading-5 text-[13px] font-light text-[#aaaaaa]'>{item.releaseDate || 'N/A'}</p>
                                        <span className='dot'></span>
                                        <p className='leading-5 text-[13px] font-medium group-hover:text-[#ffbade]'>{item.showType || 'N/A'}</p>
                                        <span className='dot'></span>
                                        <p className='leading-5 text-[13px] font-light text-[#aaaaaa]'>{item.duration || 'N/A'}</p>
                                    </div>
                                )}
                            </div>
                        </Link>
                    ...
                    {!loading && hasFetched &&
                        <Link className='w-full flex py-4 justify-center items-center bg-[#ffbade]'
