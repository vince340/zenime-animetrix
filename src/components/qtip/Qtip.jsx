...
import { faPlay, faStar, faClosedCaptioning, faMicrophone } from "@fortawesome/free-solid-svg-icons";
    ...
    <div className="w-[320px] h-fit rounded-xl p-4 flex justify-center items-center bg-[#3e3c50] bg-opacity-70 backdrop-blur-[10px] z-50">
      {loading || error ? (
        ...
        <div className="w-full flex flex-col justify-start gap-y-2">
          <h1 className="text-xl font-semibold text-white text-[13px] leading-6">{qtip.title}</h1>
          <div className="w-full flex items-center relative mt-2">
            ...
            <div className="flex ml-4 gap-x-[1px] overflow-hidden rounded-md items-center h-fit">
              {qtip?.quality && (
                <div className="bg-[#FF6300] px-[7px] w-fit flex justify-center items-center py-[1px] text-black">
              ...
              {qtip?.type && (
                <div className="absolute right-0 top-0 justify-center items-center rounded-sm bg-[#FF6300] px-[6px] text-black">
                  <p className="font-semibold text-[13px]">{qtip.type}</p>
                </div>
              )}
            </div>
          </div>
          {qtip?.description && <p className="text-[#d7d7d8] text-[13px] leading-4 font-light line-clamp-3 mt-1">{qtip.description}</p>}
              ...
              <div className="leading-4 flex flex-wrap text-wrap">
                ...
                {qtip.genres.map((genre, index) => (
                  <Link
                    to={`/genre/${genre}`}
                    key={index}
                    className="text-[13px] hover:text-[#FF6300]"
                  >
          ...
          <Link to={qtip.watchLink} className='w-[80%] flex mt-4 justify-center items-center gap-x-2 bg-[#FF6300] py-[9px] rounded-3xl'>
            <FontAwesomeIcon
              icon={faPlay}
