...
function Footer() {
    ...
    return (
        <footer className='flex flex-col w-full mt-[100px] px-4 max-[500px]:px-0'>
            ...
            >
                <img src="https://iili.io/2H76i57.png" alt={logoTitle} className='w-[200px] h-[100px]' />
            </div>
            <div className='flex py-5 flex-col w-full space-y-4 max-md:items-center max-[500px]:bg-[#373646]'>
                <div className='flex w-fit items-center space-x-6 max-[500px]:hidden'>
                    ...
                    <p style={{ borderLeft: "1px solid rgba(255, 255, 255, 0.6)" }} className='text-md font-semibold pl-6'>Searching anime order by alphabet name A to Z</p>
                </div>
                ...
                <div className='flex flex-col w-full text-left space-y-2 pt-4 max-md:items-center max-[470px]:px-[5px]'>
                    <p className='text-[#9B9BA3] text-[16px] max-md:text-center max-md:text-[12px]'>{website_name} does not store any files on our server, we only linked to the media which is hosted on 3rd party services.</p>
                    <p className='text-[#9B9BA3] max-md:text-[14px]'>© {website_name}. All rights reserved.</p>
                </div>
            </div>
        </footer>
